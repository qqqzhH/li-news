...
## 2026-09-26 深夜：第五阶段维护循环第一轮

**原因**：按维护循环巡检，发现 9/25 push OOM 的修复参数未固化进脚本（裸 `git push` 会在未来大包操作时复发）。
**服务器**：daily-collection.sh v4——push 固化 `pack.threads=1 / windowMemory=128m`；头部注释补 v3/v4 变更史；服务器仓库拉平至最新代码提交。
**资源核查**：5 个业务服务常驻约 800MB、swap 4G 全空、earlyoom 在位（--prefer node）——内存紧凑但健康，属正常业务承载非泄漏。
**测试**：bash -n + dry-run 冒烟 + 明早 9:00 cron 实战（首个 NEW_ITEMS_COUNT 数据点）。
**遗留**：无新增。本轮除该项外没有值得修改代码的问题（诚实结论）。
